...
def compress_pdf():
    ...
    try:
        ...
        # Run Ghostscript with improved quality flags
        gs_cmd = [
            "gs",
            "-sDEVICE=pdfwrite",
        ...
        # Output of QPDF
        qpdf_output = tempfile.mktemp(suffix=".pdf")
        ...
        logging.info("Running QPDF optimization...")
        subprocess.run(qpdf_cmd, check=True)
        logging.info(f"QPDF output: {qpdf_output}")

    except subprocess.CalledProcessError as e:
        logging.error(f"Compression process failed: {e}")
